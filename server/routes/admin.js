...
router.post('/users', requireAuth, requireRole('admin'), (req, res) => {
  const { employee_id, name, department, lab, role = 'staff', password } = req.body;
  ...
  const result = db.prepare(
    'INSERT INTO users (employee_id, name, department, lab, role, password_hash) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(employee_id, name, department, lab, role, hash);

  res.status(201).json({ success: true, data: { id: result.lastInsertRowid, employee_id, name, department, lab, role } });
});

// GET /api/admin/users - 사용자 목록
router.get('/users', requireAuth, requireRole('admin'), (req, res) => {
  const users = db.prepare(
    'SELECT id, employee_id, name, department, lab, role, is_active, created_at FROM users ORDER BY created_at'
  ).all();
...
router.put('/users/:id', requireAuth, requireRole('admin'), (req, res) => {
  const { name, department, lab, role, is_active, password } = req.body;
