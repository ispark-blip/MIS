          ...
          <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(expanded === lab.lab ? null : lab.lab)}>
            <div>
              ...
              <div className="flex items-baseline gap-3 mt-1">
                <span className="text-2xl font-bold">{lab.today}<span className="text-sm text-gray-500 ml-0.5">명</span></span>
                <span className="text-xs text-gray-500">월 {lab.monthlyTotal}명</span>
                <span className="text-xs text-gray-500">연 {lab.annualTotal}명</span>
              </div>
            </div>
                ...
                <tbody>
                  {lab.departments.map((d) => (
                    <tr key={d.department} className="border-t border-gray-100">
                      ...
                      <td className="text-right py-1 font-medium">{d.total}명</td>
                    </tr>
                  ))}
                </tbody>
              </table>
